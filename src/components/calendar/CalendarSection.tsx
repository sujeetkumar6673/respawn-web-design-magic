...
const CalendarSection: React.FC<CalendarSectionProps> = ({ eventDates }) => {
  ...
  return (
    <div className={`rounded-xl ${isMobile ? 'bg-transparent p-0' : 'bg-rezilia-lightblue p-4 shadow-sm'}`}>
      {!isMobile && (
      ...
      <CalendarUI
        ...
        className={`border-none rounded-lg ${isMobile ? 'mobile-calendar bg-transparent p-0' : 'bg-white p-2 shadow-sm'} pointer-events-auto`}
        modifiers={{
          ...
          DayContent: ({ date }) => {
            ...
            return (
              <div className="relative flex items-center justify-center w-full h-full">
                ...
                {hasEvents && (
                  <span className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 bg-rezilia-blue rounded-full" />
                )}
              </div>
            );
          }
